...
4. **Third pass (mono tone)**: the user sent 7 more images in two follow-up messages. Three (icon alone, horizontal, stacked-no-tagline) were re-exports of assets already integrated in pass 2 — visually identical, not re-integrated (minor pixel/crop differences from re-export, not a new design). Three more (an "outline" treatment — coral circle fill, navy line art) had a broken wordmark: the text was exported in a color that's invisible against a light/transparent background (near-white), so it wasn't usable as-is and was **not** wired in — flag to the user if they want it fixed/re-exported. The seventh, a clean **monochrome (solid rust) edition** of the full stacked lockup, was usable and became `tone="mono"` on `LogoMark` and `LogoStacked` — the icon-only and no-tagline crops for `mono` were derived locally (same row-gap-detection crop technique as elsewhere in this file) since the user only sent the single full (icon+title+tagline) mono asset. There is no `mono` horizontal (`Logo`) variant — no source asset for it; `Logo`'s `tone` stays `"light"|"dark"` only.

All eleven images (mark/logo/stacked-with-tag/stacked-no-tag × light/dark, plus mark/stacked-with-tag/stacked-no-tag × mono) are inlined as base64 `data:` URIs directly in `design-system/src/components/Logo.tsx` — no separate asset files, no build-time asset loader needed. This keeps the design-sync bundle self-contained but does mean the component's source file is large (~370KB) and **must be regenerated by script, never hand-edited** — see the git history for the generator pattern (read PNGs → base64 → template-format into the `.tsx`) if the assets ever need swapping again.

- The still-unintegrated "outline" variant (broken wordmark export) and its color values are documented above — don't rebuild it from scratch if the user re-sends it; check whether the re-export fixed the wordmark color first.
